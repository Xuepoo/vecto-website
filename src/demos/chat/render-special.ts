...
async function renderMermaid(code: string, maxWidth: number): Promise<Entity | null> {
  if (!_mermaidReady) {
...
async function renderMath(code: string, maxWidth: number): Promise<Entity | null> {
  if (!_katexReady) {
    loadStyle('https://cdn.jsdelivr.net/npm/katex@0.16/dist/katex.min.css');
    await loadScript('https://cdn.jsdelivr.net/npm/katex@0.16/dist/katex.min.js');
    _katexReady = true;
  }
...
async function renderAbc(code: string, maxWidth: number): Promise<Entity | null> {
  if (!_abcjsReady) {
    await loadScript('https://cdn.jsdelivr.net/npm/abcjs@6/dist/abcjs-basic-min.js');
    _abcjsReady = true;
  }
  ...
  for (const el of clone.querySelectorAll<SVGElement>('path,line,ellipse,text,rect,polyline')) {
