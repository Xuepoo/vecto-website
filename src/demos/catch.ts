/**
 * Fruit Catch — an osu!Catch-style challenge rendered entirely by VectoJS. The
 * whole game (falling fruit, the catcher, the goal, the HUD, and the win/lose
 * screen) is a single `Entity` drawn to one `<canvas>` through the engine's
 * `IRenderer`. There are ZERO DOM game elements — input is mouse-move + arrow
 * keys, so the demo needs no buttons at all.
 *
 * Rule: a random goal each round (e.g. "Catch 3 Apples"). You have a budget of
 * five catches — a target fills the goal, a wrong fruit wastes a catch. Reach the
 * count to pass; run out of the five first to fail.
 */
...
class CatchGame extends Entity {
  ...
  private spawn(): void {
    ...
    this.fruits.push({ x: r + Math.random() * (this.W - 2 * r), y: -r, r, key });
  }
  ...
  private drawHUD(r: IRenderer): void {
    ...
    r.fillText(`Catch ${this.goalNeed}`, 54, 30, '700 15px Inter, system-ui', '#f1f5f9');
...
function initCatch(): void {
  const canvas = document.getElementById('catch-canvas') as HTMLCanvasElement | null;
  ...
  // Keep the loop live while the stage is on-screen (play animates fruit; the ready
  // and win/lose screens have a gentle pulse). It's paused when scrolled away.
  keepSceneLive(scene, () => visible);

  const fit = (): void => {
    scene.resize(stage.clientWidth, stage.clientHeight);
    game.setSize(stage.clientWidth, stage.clientHeight);
    scene.markDirty();
  };

  const scenePt = (clientX: number, clientY: number): { x: number; y: number } => {
    const rect = canvas.getBoundingClientRect();
    return {
      x: (clientX - rect.left) * (game.W / rect.width),
      y: (clientY - rect.top) * (game.H / rect.height),
    };
  };

  canvas.addEventListener('pointermove', (e) => game.setPlate(scenePt(e.clientX, e.clientY).x));
  canvas.addEventListener('pointerdown', (e) => {
    const p = scenePt(e.clientX, e.clientY);
    if (game.phase === 'ready') {
      if (game.hitStart(p.x, p.y)) game.begin();
    } else if (game.phase === 'play') {
      game.setPlate(p.x);
    } else {
      game.reset(); // win / fail → back to the ready screen with a fresh goal
    }
  });

  // Arrow keys act only while the pointer is over the stage, so the demo never
  // hijacks the page's own arrow-key scrolling when the reader is elsewhere.
  let hovered = false;
  ...
  window.addEventListener('keyup', (e) => {
    if (e.key === 'ArrowLeft' && game.keyDir === -1) game.keyDir = 0;
    if (e.key === 'ArrowRight' && game.keyDir === 1) game.keyDir = 0;
  });
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initCatch);
else initCatch();
